...
# ... rest of your code remains the same until line 124 ...
...
# Prepare inputs for ONNX Runtime (convert PyTorch tensors to NumPy arrays)
# Move tensors to CPU first if they're on CUDA
ort_inputs = {name: tensor.cpu().numpy() for name, tensor in inputs.items()}

# Create CPU session with ONNX model
...
# Create CUDA session if available
cuda_session = None
if 'CUDAExecutionProvider' in ort.get_available_providers():
    cuda_session = ort.InferenceSession(onnx_path, providers=['CUDAExecutionProvider'])
...
if args.use_tensorrt and 'TensorrtExecutionProvider' in ort.get_available_providers():
    ...
    try:
        # TensorRT specific provider options
        provider_options = [
            {
                'device_id': 0,
                ...
                'trt_engine_cache_path': '.'           # Cache in current directory
            }
        ]
        ...
        print("   Continuing without TensorRT support.")
        tensorrt_session = None
...
if tensorrt_session is not None and args.use_tensorrt:
    ...
    try:
        ...
        # If we get here, TensorRT is working correctly
...
# Add ORT CPU results if available
if ort_cpu_prediction is not None:
    benchmark_data.append({
        'Runtime': 'ORT',
        'Provider': 'CPU',
...
# Add PyTorch GPU results if available
if has_cuda:
    benchmark_data.append({
        'Runtime': 'PyTorch',
        'Provider': 'CUDA',
...
# Add ONNX CUDA results if available
if cuda_session is not None:
    benchmark_data.append({
        'Runtime': 'ONNX',
        'Provider': 'CUDA',
...
# Add ORT CUDA results if available
if ort_cuda_prediction is not None:
    benchmark_data.append({
        'Runtime': 'ORT',
        'Provider': 'CUDA',
...
# Add TensorRT results if available
if tensorrt_usable and onnx_tensorrt_prediction is not None:
    benchmark_data.append({
        'Runtime': 'ONNX',
        'Provider': 'TensorRT',
        ...
        'Speedup vs PyTorch GPU': f"{pytorch_gpu_time/onnx_tensorrt_time:.2f}x",
...
with open(csv_filename, 'w', newline='') as csvfile:
    # First write system information
    ...
    # Then write benchmark data
    if benchmark_data:
        # Get all unique keys from all entries to handle different columns in different rows
        all_fieldnames = set()
